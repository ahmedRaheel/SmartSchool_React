import { LucideIcon,
  BarChart3, Bell, BookOpen, Bot, BriefcaseBusiness, Bus,
  CalendarCheck, ClipboardCheck, FileCheck2, GraduationCap,
  LayoutDashboard, Library, MessageCircle, Route, Settings,
  ShieldCheck, Users, Wallet, Workflow, Building2,
  Cpu, ScrollText, Zap, Globe, CreditCard, Package, Brain, BookMarked,
  MapPin, Sparkles, UserCircle, Briefcase, FileText,
  Star, ChartNoAxesCombined } from "lucide-react";

export interface NavigationItem {
  path: string; label: string; icon: LucideIcon; badge?: string; end?: boolean;
}
...
const MENUS: Record<string, { title: string; sections: NavigationSection[] }> = {
  ...
  // ── Principal — view-heavy, approval authority ─────────────────────────────
  principal: {
    title: "Principal",
    sections: [
      {
        ...
        items: [
          D,
          { path: "/reports",        label: "School Performance",  icon: ChartNoAxesCombined },
          { path: "/ai",             label: "AI Insights",         icon: Sparkles },
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          { path: "/hr",             label: "Staff List",          icon: Users },
          { path: "/finance",        label: "Fee Overview",        icon: Wallet },
          { path: "/audit",          label: "Audit Logs",          icon: ShieldCheck },
  ...
  // ── Teacher — own classroom only, no finance/HR ───────────────────────────
  teacher: {
...
export function navigationForRoles(roles: readonly string[]): NavigationSection[] {
  return (MENUS[resolveMenuKey(roles)] ?? MENUS["admin"]).sections;
}

...
export function AddEmployeeDialog({ tenantId, onClose, onCreated }: AddEmployeeDialogProps) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      for (const requirement of requirements) {
        ...
        if (!file) continue;
        await documentApi.upload({ tenantId, schoolId: form.schoolId, branchId: form.branchId, entityType: actorType, entityId: employee.id, purpose: "ONBOARDING", category: "HR", documentType: requirement.documentType, requiredDocumentTypeId: requirement.requiredDocumentTypeId, title: requirement.displayName, file });
      }
      onCreated();
    } catch (e: any) {
  ...
  return <div className="workflow-overlay"><section className="workflow-dialog actor-wizard" role="dialog" aria-modal="true">
    ...
    <form onSubmit={submit}><div className="workflow-body">
      ...
      {step === 1 && <Section title="Personal information" text="Identity information used for HR verification and account provisioning."><div className="form-grid"><Field label="First name *" value={form.firstName} onChange={(v) => set("firstName", v)}/><Field label="Last name" value={form.lastName} onChange={(v) => set("lastName", v)}/><PkCnicInput label="CNIC / national ID *" value={form.cnicNumber} onChange={(v) => set("cnicNumber", v)} required /><PkEmailInput label="Email" value={form.email} onChange={(v) => set("email", v)} /></div></Section>}
      {step === 2 && <Section title="Contact & emergency information" text="A complete contact profile is required before the actor can be approved."><div className="form-grid"><PkPhoneInput label="Primary phone *" value={form.phone} onChange={(v) => set("phone", v)} required />
<PkPhoneInput label="Alternate phone" value={form.alternatePhone} onChange={(v) => set("alternatePhone", v)} />
<PkPhoneInput label="Emergency phone *" value={form.emergencyContactPhone} onChange={(v) => set("emergencyContactPhone", v)} required />
